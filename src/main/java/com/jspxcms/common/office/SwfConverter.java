...
public class SwfConverter {
    private static final Logger logger = LoggerFactory.getLogger(SwfConverter.class);

    public static void pdf2swf(File from, File to, String exe, String languagedir) throws Exception {
        ...
        String[] command = {exe, from.getPath(), "-o", to.getPath(), "-f", "-T", "9", "-t", "-v", "-s", "storeallcharacters", "-s", "poly2bitmap"};
        if (StringUtils.isNotBlank(languagedir)) {
            command = ArrayUtils.addAll(command, "-s", "languagedir=" + languagedir);
        }
