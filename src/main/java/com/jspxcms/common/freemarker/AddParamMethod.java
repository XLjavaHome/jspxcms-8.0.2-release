...
public class AddParamMethod implements TemplateMethodModelEx {
    ...
    public Object exec(List args) throws TemplateModelException {
        ...
        // 获取地址
        String dyUrl = Freemarkers.getString((TemplateModel) args.get(0), "arg0");
        // 获取参数名称
        String name = Freemarkers.getString((TemplateModel) args.get(1), "arg1");
        // 获取参数值，可以有多个。
        List<String> values = new ArrayList<String>(argsSize - 1);
        for (int i = 2; i < argsSize; i++) {
            String value = Freemarkers.getString((TemplateModel) args.get(i), "arg" + i);
            if (value != null) {
                values.add(value);
            }
        }
        ...
        // 如果是静态页，则用dyUrl。
        if (StringUtils.isNotBlank(dyUrl) && (url.endsWith(".html") || url.endsWith(".htm"))) {
            url = dyUrl;
        }
        String queryString = "";
        String uri = url;
        ...
        if (m.find()) {
            queryString = queryString.substring(0, m.start(1)) + queryString.substring(m.end(1));
        }
        // 去除参数分页
