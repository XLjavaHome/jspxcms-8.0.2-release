...
public class ImageMagickHandler implements ImageHandler {
    ...
    public boolean composite(String overlay, String src, String dest, WatermarkParam watermarkParam) {
        ...
        try {
            ...
            if (width < watermarkParam.getMinWidth() || height < watermarkParam.getMinHeight() || watermarkWidth + watermarkParam.getPaddingX() > width || watermarkHeight + watermarkParam.getPaddingY() > height) {
                return false;
            }
            Gravity gravity = Gravity.values()[watermarkParam.getPosition() - 1];
            return composite(overlay, src, dest, gravity, watermarkParam.getPaddingX(), watermarkParam.getPaddingY(), watermarkParam.getAlpha());
        } catch (Exception e) {
            FileUtils.deleteQuietly(new File(dest));
            throw new RuntimeException(e);
        }
