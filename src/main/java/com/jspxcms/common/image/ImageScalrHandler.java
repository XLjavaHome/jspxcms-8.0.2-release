...
public class ImageScalrHandler implements ImageHandler {
    ...
    public boolean resize(String src, String dest, Integer width, Integer height, boolean exact) {
        ...
        try {
            ...
            int srcWidth = srcBuff.getWidth();
            int srcHeight = srcBuff.getHeight();
            if ((height == null && width >= srcWidth) || (width == null && height >= srcHeight) || (height != null && width != null && height >= srcHeight && width >= srcWidth)) {
                return false;
            }
            ...
            if (height == null) {
                ...
                destBuff = Scalr.resize(srcBuff, Scalr.Method.QUALITY, exact ? Scalr.Mode.FIT_EXACT : Scalr.Mode.AUTOMATIC, width, height);
            }
            FilesEx.makeParentDir(destFile);
            ImageIO.write(destBuff, formatName, destFile);
            return true;
    ...
    public boolean composite(String overlay, String src, String dest, WatermarkParam watermarkParam) {
        ...
        try {
            ...
            // 水印超出原图范围
            if (width < minWidth || height < minHeight || overlayWidth + paddingX > width || overlayHeight + paddingY > height) {
                return false;
            }
            int x, y;
            switch (position) {
