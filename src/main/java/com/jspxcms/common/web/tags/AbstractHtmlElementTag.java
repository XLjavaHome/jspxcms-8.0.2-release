 ...
 * @author liufang
 */
public abstract class AbstractHtmlElementTag extends RequestContextAwareTag implements DynamicAttributes {
    ...
    protected abstract int writeTagContent(TagWriter writer) throws JspException;
    ...
    protected void writeOptionalAttributes(TagWriter w) throws JspException {
        ...
        if (!CollectionUtils.isEmpty(this.dynamicAttributes)) {
            for (String attr : this.dynamicAttributes.keySet()) {
                String displayValue = ObjectUtils.getDisplayString(this.dynamicAttributes.get(attr));
                w.writeOptionalAttributeValue(attr, HtmlUtils.htmlEscape(displayValue));
            }
        }
    }
