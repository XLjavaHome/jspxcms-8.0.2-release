...
public abstract class QuerydslUtils {
    ...
    public static <T> List<T> list(JPAQuery<T> query, EntityPathBase<T> entityPath, Limitable limitable) {
    ...
    public static <T> Page<T> page(JPAQuery<T> query, EntityPathBase<T> entityPath, Pageable pageable) {
    ...
    public static <T, Q extends SimpleQuery<Q>> void applySorting(SimpleQuery<Q> query, EntityPath<T> entityPath, Sort sort) {
        if (sort == null) {
            return;
        }
