...
public class GuestbookController {
    ...
    public String index(Integer page, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return index(null, page, request, response, modelMap);
    }

    @RequestMapping(Constants.SITE_PREFIX_PATH + "/guestbook")
    public String index(@PathVariable String siteNumber, Integer page, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
    ...
    public String show(@PathVariable Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return show(null, id, request, response, modelMap);
    }

    @RequestMapping(Constants.SITE_PREFIX_PATH + "/guestbook/{id}")
    public String show(@PathVariable String siteNumber, @PathVariable Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        siteResolver.resolveSite(siteNumber);
        Response resp = new Response(request, response, modelMap);
        Site site = Context.getCurrentSite();
        SiteGuestbook sg = new SiteGuestbook(site.getCustoms());
    ...
    public String createForm(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return createForm(null, request, response, modelMap);
    }

    @RequestMapping(Constants.SITE_PREFIX_PATH + "/guestbook/create")
    public String createForm(@PathVariable String siteNumber, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        siteResolver.resolveSite(siteNumber);
        Response resp = new Response(request, response, modelMap);
        Site site = Context.getCurrentSite();
    ...
    public String createSubmit(Integer typeId, String text, String title, String username, Boolean gender, String phone, String mobile, String qq, String email, String captcha, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return createSubmit(null, typeId, text, title, username, gender, phone, mobile, qq, email, captcha, request, response, modelMap);
    }

    @RequestMapping(value = {Constants.SITE_PREFIX_PATH + "/guestbook/create", Constants.SITE_PREFIX_PATH + "/guestbook"}, method = RequestMethod.POST)
    public String createSubmit(@PathVariable String siteNumber, Integer typeId, String text, String title, String username, Boolean gender, String phone, String mobile, String qq, String email, String captcha, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        siteResolver.resolveSite(siteNumber);
        Response resp = new Response(request, response, modelMap);
        Site site = Context.getCurrentSite();
        SiteGuestbook sg = new SiteGuestbook(site.getCustoms());
        String result = validateSubmit(request, resp, sg, captcha, typeId, title, text, username, gender, phone, mobile, qq, email);
        if (resp.hasErrors()) {
            return result;
        }
    ...
    private String validateSubmit(HttpServletRequest request, Response resp, SiteGuestbook sg, String captcha, Integer typeId, String title, String text, String username, Boolean gender, String phone, String mobile, String qq, String email) {
        List<String> messages = resp.getMessages();
        if (!Validations.notEmpty(text, sg.getMaxLength(), messages, "text")) {
            return resp.post(401);
        }
        if (typeId != null) {
            if (!Validations.exist(guestbookTypeService.get(typeId), messages, "GuestbookType", typeId)) {
                resp.post(451);
            }
        }
        User user = Context.getCurrentUser();
