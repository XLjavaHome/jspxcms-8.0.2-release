...
public class QuestionController {
    ...
    public String show(@PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return show(null, id, request, response, modelMap);
    }

    @RequestMapping(Constants.SITE_PREFIX_PATH + "/questions/{id}")
    public String show(@PathVariable String siteNumber, @PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        siteResolver.resolveSite(siteNumber);
    ...
    public String result(@PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return result(null, id, request, response, modelMap);
    }

    @RequestMapping(Constants.SITE_PREFIX_PATH + "/questions/{id}/result")
    public String result(@PathVariable String siteNumber, @PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        siteResolver.resolveSite(siteNumber);
        Response resp = new Response(request, response, modelMap);
        List<String> messages = resp.getMessages();
        Question question = service.get(id);
    ...
    public String answer(@PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        return answer(null, id, request, response, modelMap);
    }

    @RequestMapping(value = Constants.SITE_PREFIX_PATH + "/questions/{id}/answer", method = RequestMethod.POST)
    public String answer(@PathVariable String siteNumber, @PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        siteResolver.resolveSite(siteNumber);
        Response resp = new Response(request, response, modelMap);
        Integer userId = Context.getCurrentUserId();
        String ip = Servlets.getRemoteAddr(request);
        String cookie = Site.getIdentityCookie(request, response);
        Map<String, String[]> selects = Servlets.getParamValuesMap(request, "item_");
        ...
        Integer[] optionIds = optionIdList.toArray(new Integer[optionIdList.size()]);
        Map<String, String> inputStrings = Servlets.getParamMap(request, "input_");
        Map<Integer, String> inputs = new HashMap<Integer, String>(inputStrings.size());
        for (Entry<String, String> entry : inputStrings.entrySet()) {
            inputs.put(Integer.parseInt(entry.getKey()), entry.getValue());
        }
        String result = validateAnswer(request, resp, id, optionIds, inputs, userId, ip, cookie);
        if (resp.hasErrors()) {
            return result;
        }
    ...
    private String validateAnswer(HttpServletRequest request, Response resp, Integer id, Integer[] optionIds, Map<Integer, String> inputs, Integer userId, String ip, String cookie) {
        List<String> messages = resp.getMessages();
        ...
        if (question.getBeginDate() != null && question.getBeginDate().getTime() > System.currentTimeMillis()) {
            return resp.post(502, "question.notBegin");
        }
        if (question.getEndDate() != null && question.getEndDate().getTime() < System.currentTimeMillis()) {
