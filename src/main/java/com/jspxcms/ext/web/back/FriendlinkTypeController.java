...
public class FriendlinkTypeController {
    ...
    public String save(FriendlinkType bean, String redirect, HttpServletRequest request, RedirectAttributes ra) {
        Integer siteId = Context.getCurrentSiteId();
        service.save(bean, siteId);
        logService.operation("opr.friendlinkType.add", bean.getName(), null, bean.getId(), request);
    ...
    public String batchUpdate(Integer[] id, String[] name, String[] number, HttpServletRequest request, RedirectAttributes ra) {
        Site site = Context.getCurrentSite();
        if (ArrayUtils.isNotEmpty(id)) {
            validateIds(id, site.getId());
            FriendlinkType[] beans = service.batchUpdate(id, name, number);
            for (FriendlinkType bean : beans) {
                logService.operation("opr.friendlinkType.batchEdit", bean.getName(), null, bean.getId(), request);
    ...
    public String delete(Integer[] ids, HttpServletRequest request, RedirectAttributes ra) {
        ...
        for (FriendlinkType bean : beans) {
            logService.operation("opr.friendlinkType.delete", bean.getName(), null, bean.getId(), request);
