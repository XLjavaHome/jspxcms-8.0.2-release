...
public class GuestbookTypeController {
    ...
    public String save(GuestbookType bean, String redirect, HttpServletRequest request, RedirectAttributes ra) {
        Integer siteId = Context.getCurrentSiteId();
        service.save(bean, siteId);
        logService.operation("opr.guestbookType.add", bean.getName(), null, bean.getId(), request);
    ...
    public String batchUpdate(Integer[] id, String[] name, String[] number, String[] description, HttpServletRequest request, RedirectAttributes ra) {
        ...
        for (GuestbookType bean : beans) {
            logService.operation("opr.guestbookType.batchEdit", bean.getName(), null, bean.getId(), request);
    ...
    public String delete(Integer[] ids, HttpServletRequest request, RedirectAttributes ra) {
        ...
        for (GuestbookType bean : beans) {
            logService.operation("opr.guestbookType.delete", bean.getName(), null, bean.getId(), request);
