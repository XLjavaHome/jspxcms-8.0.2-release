...
public class CollectorImpl implements Collector {
    ...
    public class CollectThread extends Thread {
        ...
        public void run() {
            ...
            CloseableHttpClient httpclient = HttpClients.custom().setUserAgent(collect.getUserAgent()).build();
            try {
        ...
        private void doCollect(CloseableHttpClient httpclient, Collect collect) throws ClientProtocolException, IOException, URISyntaxException {
            ...
            for (URI listUri : listUris) {
                ...
                for (URI itemUri : itemUris) {
                    ...
                    boolean success = service.collcetItem(httpclient, itemUri, collectId, charset, nodeId, creatorId, info, detail, customs, clobs);
                    String status = Info.COLLECTED;
                    if (collect.getSubmit()) {
                        status = Info.NORMAL;
                    }
                    if (success) {
                        infoService.save(info, detail, null, null, null, null, customs, clobs, null, null, null, null, null, nodeId, creatorId, status, siteId);
                    }
                }
            }
        }
