...
public class MessageDaoImpl implements MessageDaoPlus {
    ...
    private JpqlBuilder groupByUserId(Integer userId, boolean unread) {
        ...
        jb.append(
                "select max(t.message_id_) as message_id_,                                                          " + "       t.contact_id_ as contact_id_,                                                               " + "       u.f_username as contact_username_,                                                          " + "       max(t.send_time_) as send_time_,                                                            " + "       count(t.contact_id_) as number_of_messages_,                                                " + "       sum(t.unread_) as number_of_unread_                                                         " + "from (                                                                                             " + "  select m.message_id_,m.send_time_,m.receiver_id_ as contact_id_,0 as unread_                     " + "  from cms_message m                                                                               " + "  where m.sender_id_=:userId and m.deletion_flag_<>:deletionSend                                   " + "  UNION                                                                                            " + "  select m.message_id_,m.send_time_,m.sender_id_ as contact_id_,m.is_unread_ as unread_            " + "  from cms_message m                                                                               " + "  where m.receiver_id_=:userId and m.deletion_flag_<>:deletionReceive                              " + ") t join cms_user u on t.contact_id_=u.f_user_id                                                   " + "group by t.contact_id_, u.f_username                                                               ");
        if (unread) {
    ...
    private JPAQuery<Message> findByContactId(Integer userId, Integer contactId, QMessage m) {
        ...
        query.from(m);
        BooleanBuilder senderExp = new BooleanBuilder(m.sender.id.eq(userId)).and(m.deletionFlag.ne(Message.DELETION_SEND));
        BooleanBuilder receiverExp = new BooleanBuilder(m.receiver.id.eq(userId)).and(m.deletionFlag.ne(Message.DELETION_RECEIVE));
