...
public class SiteResolver {
     ...
     * 解析当前站点。
     *
     * @param siteNumber 站点代码
     * @param siteable   带站点信息的对象
     * @return 解析所得的Site对象
     */
    public void resolveSite(String siteNumber, Siteable siteable) {
        ...
        if (siteNumber != null) {
            ...
            if (site == null) {
                throw new IllegalStateException("Site number not found: " + siteNumber);
            }
        ...
        if (siteable != null) {
            if (!siteable.getSite().getId().equals(site.getId())) {
                ...
                throw new CmsException("error.dataNotInSite", className, dataId, dataSiteId, siteId);
            }
        }
    }
