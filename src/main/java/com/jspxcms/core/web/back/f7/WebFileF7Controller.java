...
public class WebFileF7Controller {
    ...
    public String f7TemplateTree(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) throws IOException {
        Site site = Context.getCurrentSite();
        String displayPath = site.getGlobal().getTemplateDisplayPathByCtx();
        File rootFile = new File(pathResolver.getPath("", Constants.TEMPLATE_STORE_PATH));
        // 默认父文件夹为file基础路径
        String base = site.getSiteBase(site.getTemplateTheme());
        File baseFile = new File(pathResolver.getPath(base, Constants.TEMPLATE_STORE_PATH));
        // 当前选中路径
        String id = Servlets.getParam(request, "id");
        WebFile bean = null;
        if (StringUtils.isNotBlank(id)) {
            File idFile = new File(pathResolver.getPath(id, Constants.TEMPLATE_STORE_PATH));
            bean = new WebFile(idFile, rootFile.getCanonicalPath(), displayPath);
        }
        WebFile baseWebFile = new WebFile(baseFile, rootFile.getCanonicalPath(), displayPath);
    ...
    public String f7StyleTree(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) throws IOException {
        Site site = Context.getCurrentSite();
        String displayPath = site.getGlobal().getTemplateDisplayPathByCtx();
        File rootFile = new File(pathResolver.getPath("", Constants.TEMPLATE_STORE_PATH));
        // 默认父文件夹为filesUrl
    ...
    public String f7Uploads(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) throws IOException {
        Site site = Context.getCurrentSite();
    ...
    public String f7UploadsList(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) throws IOException {
