...
public class AttributeController {
    ...
    public String save(Attribute bean, HttpServletRequest request, RedirectAttributes ra) {
        Integer siteId = Context.getCurrentSiteId();
        service.save(bean, siteId);
        logService.operation("opr.attribute.add", bean.getName(), null, bean.getId(), request);
    ...
    public String batchUpdate(Integer[] id, String[] name, String[] number, Integer[] imageWidth, Integer[] imageHeight, Boolean[] scale, Boolean[] exact, Boolean[] watermark, HttpServletRequest request, RedirectAttributes ra) {
        Site site = Context.getCurrentSite();
        validateIds(id, site.getId());
        Attribute[] beans = service.batchUpdate(id, name, number, imageWidth, imageHeight, scale, exact, watermark);
        for (Attribute bean : beans) {
            logService.operation("opr.attribute.batchEdit", bean.getName(), null, bean.getId(), request);
    ...
    public String delete(Integer[] ids, HttpServletRequest request, RedirectAttributes ra) {
        ...
        for (Attribute bean : beans) {
            logService.operation("opr.attribute.delete", bean.getName(), null, bean.getId(), request);
    ...
    @ResponseBody
    public String checkNumber(String number, String original, HttpServletRequest request) {
