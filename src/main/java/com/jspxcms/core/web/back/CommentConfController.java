...
public class CommentConfController {
    private static final Logger logger = LoggerFactory.getLogger(CommentConfController.class);
    ...
    public String update(SiteComment bean, HttpServletRequest request, RedirectAttributes ra) {
        Site site = Context.getCurrentSite();
        siteService.updateConf(site, bean);
        logService.operation("opr.commentConf.edit", site.getName(), null, site.getId(), request);
