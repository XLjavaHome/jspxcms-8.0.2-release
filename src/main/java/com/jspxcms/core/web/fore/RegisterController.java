...
public class RegisterController {
    ...
    public String registerForm(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
    ...
    public String registerSubmit(String captcha, String username, String password, String email, String gender, Date birthDate, String bio, String comeFrom, String qq, String msn, String weixin, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
        Site site = Context.getCurrentSite();
        ...
        User user = userService.register(ip, groupId, orgId, status, username, password, email, null, null, null, gender, birthDate, bio, comeFrom, qq, msn, weixin);
    ...
    public String registerMessage(String email, Integer verifyMode, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
        Site site = Context.getCurrentSite();
        GlobalRegister reg = site.getGlobal().getRegister();
    ...
    public String verifyMember(String key, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
    ...
    public String forgotPasswordForm(HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Site site = Context.getCurrentSite();
        Map<String, Object> data = modelMap.asMap();
        ForeContext.setData(data, request);
    ...
    @RequestMapping(value = {"/forgot_password", Constants.SITE_PREFIX_PATH + "/forgot_password"}, method = RequestMethod.POST)
    public String forgotPasswordSubmit(String username, String email, String captcha, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
    ...
    public String retrievePasswordForm(String key, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
        List<String> messages = resp.getMessages();
        if (!Validations.notEmpty(key, messages, "key")) {
            return resp.badRequest();
        ...
        if (retrieveUser == null || retrieveUser.getValidationDate() == null || System.currentTimeMillis() - retrieveUser.getValidationDate().getTime() > 8 * 60 * 60 * 1000) {
    ...
    @RequestMapping(value = {"/retrieve_password", Constants.SITE_PREFIX_PATH + "/retrieve_password"}, method = RequestMethod.POST)
    public String retrievePasswordSubmit(String key, String password, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
        List<String> messages = resp.getMessages();
        if (!Validations.notEmpty(key, messages, "key")) {
    ...
    public String checkUsername(String username, String original, HttpServletRequest request, HttpServletResponse response) {
    ...
    private String validateRegisterSubmit(HttpServletRequest request, Response resp, GlobalRegister reg, String captcha, String username, String password, String email, String gender) {
        List<String> messages = resp.getMessages();
        ...
        if (reg.getVerifyMode() == GlobalRegister.VERIFY_MODE_EMAIL && !Validations.notEmpty(email, messages, "email")) {
    ...
    private String validateRegisterMessage(HttpServletRequest request, Response resp, GlobalRegister reg, String username, String email, Integer verifyMode) {
        List<String> messages = resp.getMessages();
        if (!Validations.notEmpty(username, messages, "username")) {
            return resp.badRequest();
        }
    ...
    private String validateForgotPasswordSubmit(HttpServletRequest request, Response resp, String username, String email, String captcha) {
        List<String> messages = resp.getMessages();
        if (!Captchas.isValid(captchaService, request, captcha)) {
            return resp.post(100, "error.captcha");
        }
