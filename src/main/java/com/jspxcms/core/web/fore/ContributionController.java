...
public class ContributionController {
    ...
    @RequestMapping(value = {"/my/contribution/update/{id}", Constants.SITE_PREFIX_PATH + "/my/contribution/update/{id}"})
    public String updateForm(@PathVariable("id") Integer id, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
    ...
    public String createSubmit(Integer nodeId, String title, String text, String file, String fileName, Long fileLength, @RequestParam(defaultValue = "false") boolean draft, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
        List<String> messages = resp.getMessages();
        Site site = Context.getCurrentSite();
        User user = Context.getCurrentUser();
        ...
        infoService.save(bean, detail, null, null, null, null, null, clobs, null, null, null, null, null, nodeId, userId, status, siteId);
        return resp.post();
    }

    @RequestMapping(value = {"/my/contribution/update", Constants.SITE_PREFIX_PATH + "/my/contribution/update"}, method = RequestMethod.POST)
    public String updateSubmit(Integer id, Integer nodeId, String title, String text, String video, String videoName, Long videoLength, String videoTime, String file, String fileName, Long fileLength, String[] filesName, String[] filesFile, Long[] filesLength, String[] imagesName, String[] imagesText, String[] imagesImage, @RequestParam(defaultValue = "false") boolean pass, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
        List<String> messages = resp.getMessages();
        User user = Context.getCurrentUser();
        Collection<MemberGroup> groups = Context.getCurrentGroups(request);
        ...
        if (!bean.getStatus().equals(Info.DRAFT) && !bean.getStatus().equals(Info.CONTRIBUTION) && !bean.getStatus().equals(Info.REJECTION)) {
        ...
        Map<String, String> clobs = new HashMap<String, String>();
        clobs.put("text", text);
        infoService.update(bean, detail, null, null, null, null, null, clobs, images, files, null, null, null, nodeId, user, pass, true);
        return resp.post();
    }

    @RequestMapping(value = {"/my/contribution/delete", Constants.SITE_PREFIX_PATH + "/my/contribution/delete"})
    public String delete(Integer[] ids, HttpServletRequest request, HttpServletResponse response, org.springframework.ui.Model modelMap) {
        Response resp = new Response(request, response, modelMap);
