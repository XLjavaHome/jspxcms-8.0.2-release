...
public class HtmlServiceImpl implements HtmlService {
    ...
    public void makeNode(Node node) {
        ...
        if (STATIC_INFO_NODE_PARENT == method || STATIC_INFO_NODE_PARENT_LIST == method) {
            while (node != null) {
                //				fileHandler = node.getSite().getHtmlPublishPoint()
                //						.getFileHandler(pathResolver);
                try {
                    PNode.makeHtml(node, max, getConfig(), pathResolver, taskService, null);
                } catch (Exception e) {
                    logger.error(null, e);
                }
                node = node.getParent();
    ...
    public Integer makeInfo(Integer siteId, Integer nodeId, boolean includeChildren, Integer taskId, boolean forUpdate, Integer lastId, int maxResult) throws IOException, TemplateException {
        String treeNumber = null;
        if (includeChildren && nodeId != null) {
            Node node = nodeQuery.get(nodeId);
            treeNumber = node.getTreeNumber();
        }
        List<Info> list = infoDao.findForHtml(siteId, nodeId, treeNumber, forUpdate, lastId, maxResult);
        //		FileHandler fileHandler;
    ...
    public Integer makeNode(Integer siteId, Integer nodeId, boolean includeChildren, Integer taskId, boolean forUpdate, Integer lastId, int maxResult) throws IOException, TemplateException {
        String treeNumber = null;
        if (includeChildren && nodeId != null) {
            Node node = nodeQuery.get(nodeId);
            treeNumber = node.getTreeNumber();
        }
        List<Node> list = nodeDao.findForHtml(siteId, nodeId, treeNumber, forUpdate, lastId, maxResult);
        //		FileHandler fileHandler;
        ...
        while (it.hasNext() && taskService.isRunning(taskId)) {
            ...
            //			fileHandler = node.getSite().getHtmlPublishPoint()
            //					.getFileHandler(pathResolver);
            PNode.makeHtml(node, Integer.MAX_VALUE, getConfig(), pathResolver, taskService, taskId);
        }
        if (list.size() < maxResult) {
            lastId = null;
        }
