...
public abstract class PNode {
    public static void makeHtml(Node node, int max, Configuration config, PathResolver pathResolver, TaskService taskService, Integer taskId) throws IOException, TemplateException {
        if (node == null) {
            return;
        }
        ...
        try {
            ...
            // 生成手机端HTML
            if (node.getSite().getMobilePublishPoint() != null && node.getSite().getMobilePublishPoint().getId() != node.getSite().getHtmlPublishPoint().getId() && StringUtils.isNotBlank(node.getSite().getMobileDomain())) {
    ...
    public static void doMakeHtml(Node node, int max, Configuration config, PathResolver pathResolver, TaskService taskService, Integer taskId) throws IOException, TemplateException {
        ...
        PublishPoint publishPoint = Context.isMobile() ? node.getSite().getMobilePublishPoint() : node.getSite().getHtmlPublishPoint();
