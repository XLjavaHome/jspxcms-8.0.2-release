...
public class HtmlGeneratorImpl implements HtmlGenerator {
    ...
    public void makeInfo(final Integer siteId, final Integer nodeId, String nodeName, final boolean includeChildren, Integer userId) {
        ...
        new Thread() {
            public void run() {
                try {
                    Integer lastId = null;
                    do {
                        lastId = htmlService.makeInfo(siteId, nodeId, includeChildren, taskId, false, lastId, INFO_MAX_RESULT);
                    } while (lastId != null);
                    taskService.finish(taskId);
                } catch (Exception e) {
                    taskService.abort(taskId);
    ...
    public void makeNode(final Integer siteId, final Integer nodeId, String nodeName, final boolean includeChildren, Integer userId) {
        if (StringUtils.isNotBlank(nodeName)) {
            nodeName = "ALL";
        }
    ...
    public void deleteHtml(Node node) {
        ...
        if (StringUtils.isNotBlank(site.getMobileDomain()) && !site.getHtmlPublishPoint().getId().equals(site.getMobilePublishPoint().getId())) {
