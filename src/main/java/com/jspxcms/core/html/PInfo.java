...
public abstract class PInfo {
    public static void makeHtml(Info info, Configuration config, PathResolver pathResolver, TaskService taskService, Integer taskId) throws IOException, TemplateException {
        if (info == null) {
            return;
        }
        ...
        try {
            ...
            // 生成手机端HTML
            if (info.getSite().getMobilePublishPoint() != null && info.getSite().getMobilePublishPoint().getId() != info.getSite().getHtmlPublishPoint().getId() && StringUtils.isNotBlank(info.getSite().getMobileDomain())) {
    ...
    private static void doMakeHtml(Info info, Configuration config, PathResolver pathResolver, TaskService taskService, Integer taskId) throws IOException, TemplateException {
        ...
        PublishPoint publishPoint = Context.isMobile() ? info.getSite().getMobilePublishPoint() : info.getSite().getHtmlPublishPoint();
