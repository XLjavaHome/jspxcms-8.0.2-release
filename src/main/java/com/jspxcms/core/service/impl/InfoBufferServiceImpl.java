...
public class InfoBufferServiceImpl implements InfoBufferService {
    ...
    public int updateViews(Integer id) {
        ...
        if (buffViews >= info.getSite().getGlobal().getOther().getBufferInfoViews()) {
            buffer.setViews(0);
    ...
    public int updateDownloads(Integer id) {
        ...
        // 根据缓冲设置
        if (buffDownloads >= info.getSite().getGlobal().getOther().getBufferInfoDownloads()) {
    ...
    public int updateDiggs(Integer id, Integer userId, String ip, String cookie) {
        ...
        // 根据缓冲设置
        if (buffDiggs >= info.getSite().getGlobal().getOther().getBufferInfoDiggs()) {
            buffer.setDiggs(0);
    ...
    @Transactional
    public int updateScore(Integer id, Integer itemId, Integer userId, String ip, String cookie) {
        Info info = infoQueryService.get(id);
        InfoBuffer buffer = get(id);
        if (buffer == null) {
            buffer = new InfoBuffer();
        ...
        if (buffScore >= info.getSite().getGlobal().getOther().getBufferInfoScore()) {
            buffer.setScore(0);
            info.setScore(score + buffScore);
        } else {
            buffer.setScore(buffScore);
        }
        ScoreBoard board = scoreBoardService.scoring(Info.SCORE_MARK, id, itemId);
