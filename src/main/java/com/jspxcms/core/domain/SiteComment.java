...
public class SiteComment implements Configurable {
    ...
    public boolean isNeedCaptcha(User user) {
        ...
        return !((captchaMode == CAPTCHA_MODE_OFF) || (captchaMode == CAPTCHA_MODE_GUEST && user != null) || (captchaMode == CAPTCHA_MODE_USER && user != null && user.isAdmin()));
    }

    public boolean isAudit(User user) {
        int auditMode = getAuditMode();
        return (auditMode == AUDIT_MODE_OFF) || (auditMode == AUDIT_MODE_GUEST && user != null) || (auditMode == AUDIT_MODE_USER && user != null && user.isAdmin());
    }
